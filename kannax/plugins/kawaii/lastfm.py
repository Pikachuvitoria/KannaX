...
async def last_fm_pic_(message: Message):
    ...
    rep += f"\n<b>🎶 Musica</b>\n<i><a href={song_['url']}>{song_name}</a></i>\n\n<b>👥 Artista</b>\n <i><a href={song_['artist']['url']}>{artist_name}</a></i>"
    if song_["loved"] != "0":
        rep += " (♥️ loved)"
...
async def lastfm_compat_(message: Message):
    ...
    rep = f"{display} ambos ouvem: \n__{disart}__...\n\nCompatibilidade de músical é de **{compat}%**"
